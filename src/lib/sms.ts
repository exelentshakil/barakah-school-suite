...
// FILE: src/lib/sms.ts - UPDATED sendBulkSMS WITH JSON RESPONSE
...
export async function sendBulkSMS(recipients: SMSMessage[]): Promise<number> {
    ...
    for (const [message, numbers] of messageGroups.entries()) {
        try {
            ...
            if (code === 202) {
                // Success - count actual recipients
                const successfulCount = numbers.length;
                sentCount += successfulCount;

                // Calculate SMS count per message
                const smsInfo = getSMSInfo(message);
                const smsUsedForThisGroup = successfulCount * smsInfo.count;
                totalSMSUsed += smsUsedForThisGroup;
                ...
                // Error
                const errorMsg = result.error_message || ERROR_CODES[code] || `Unknown error: ${code}`;
    ...
    // Deduct balance ONCE after all messages sent
    if (totalSMSUsed > 0) {
        const { data: currentCredit } = await supabase
            .from('sms_credits')
            .select('balance')
        ...
        const newBalance = currentBalance - totalSMSUsed;
    ...
    return sentCount; // Returns number of recipients, not SMS count
}
